...
export const searchOurProducts = async (param: any) => {
  console.log(param);
  const response = await axiosService.get(
    `/products/getproductsdata?search=${param}`
  );
  return response;
};
...
export const createMainAd = async (payload: any): Promise<any> => {
  try {
    const response = await axiosService.post("/mainads/create", payload);
    if (response.status === 200) {
      toast.success("Ad created...");
    } else {
      toast.success("Error creating ad..");
    }
    return response;
  } catch (error: any) {
    console.error(error);
